...
SOURCE_DIR=${0%/*}
BIN_DIR=~/Applications/iBackupFS.app
DST_DIR=/tmp/iBackupFS.$$
DST_IMG=${DST_DIR}.dmg
...
# copy READMEs
cd $SOURCE_DIR
gnutar cf - README | ( cd $DST_DIR; gnutar xf - )

# remove extra garbage
cd $DST_DIR
# some build artifact
rm -f iBackupFS.app/iBackupFS.app
...
echo "scp ${REL_IMG} ftp.mulle-kybernetik.com:~ftp/pub/software/iBackupFS/"
echo "=== DOWNLOAD URLS ==="
echo "http://www.mulle-kybernetik.com/software/iBackupFS/downloads/${REL_IMG##*/}"
echo "ftp://ftp.mulle-kybernetik.com/pub/software/iBackupFS/${REL_IMG##*/}"
echo "=== RSS FEED ==="
echo "<enclosure sparkle:md5Sum=\"${MD5SUM}\" url=\"http://www.mulle-kybernetik.com/software/iBackupFS/downloads/${REL_IMG##*/}\" length=\"${REL_IMG_SIZE_B}\" type=\"application/octet-stream\"/>"
